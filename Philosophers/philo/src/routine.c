...
/*   Updated: 2026/02/16 18:36:35 by vampaier2        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (checkdead_flag(philo->mutex_man))
		return (1);
	lock_forks(philo);
	if (checkdead_flag(philo->mutex_man))
	{
    ...
    msg(philo->mutex_man, philo->id, "is eating");
	unlock_forks(philo);
    if (philo->param.number_oftotal_meals != -1 && philo->n_meals >= philo->param.number_oftotal_meals)
        return (1);
    return (0);
}
...
{
    if (checkdead_flag(philo->mutex_man) == 1)// if any philo dead, you stop    
	    return (1);
    msg(philo->mutex_man, philo->id, "is sleeping");
